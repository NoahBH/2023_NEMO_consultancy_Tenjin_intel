  ...
  for (i in 1:length(vectors)){
    ...
    PartialData %>% group_by(person_id, period) %>%
      suppressMessages(summarise(n_data = n(),
                                 ...
                                 slow = as.logical(round(mean(slow))),
                                 at_tablet = as.logical(round(mean(at_tablet))),
                                 deviance_x = mean(deviance_x),
                                 deviance_y = mean(deviance_y),
                                 speed_x = mean(speed_x),
                                 speed_y = mean(speed_y),
                                 speed = mean(speed),
                                 acc_x = mean(acc_x),
                                 acc_y = mean(acc_y)))
